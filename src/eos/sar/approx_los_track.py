...
def get_los_vector_3D(image_geometry, incidence_angle, crs=None):
    ...
    incidence = incidence_angle * np.pi / 180.0
    los_vector_3D = np.array(
        [
            np.sin(incidence) * np.sin(heading),
            np.sin(incidence) * np.cos(heading),
            -np.cos(incidence),
        ]
    )
...
def get_track_vector(image_geometry, crs=None):
    ...
    track_vector = track_vector / np.sqrt(
        np.dot(track_vector, track_vector)
    )  # normalise
    return track_vector
