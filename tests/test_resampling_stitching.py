...
def test_burst_intersection(inputs):
...
def mat_estim(prim_mod, sec_mod, x, y, alt, crs):
    # project in primary
...
def test_burst_matrix_estimation(dem, inputs):
    image_readers, primary_bursts_meta, secondary_bursts_meta, ref_metas = inputs
    ...
    # Now instantiate burst_model instances for projection/localization
    primary_burst_model = _get_objects(primary_bursts_meta[b_index], ref_metas[0])[3]
    secondary_burst_model = _get_objects(secondary_bursts_meta[b_index], ref_metas[1])[
        3
    ]

    A = mat_estim(primary_burst_model, secondary_burst_model, *dem)
    assert np.any(A != np.eye(3))
    A_inv = mat_estim(secondary_burst_model, primary_burst_model, *dem)
    assert np.any(A_inv != np.eye(3))

    assert np.allclose(A.dot(A_inv), np.eye(3), rtol=1e-2, atol=1e-2)


def test_burst_registration(inputs):
    image_readers, primary_bursts_meta, secondary_bursts_meta, _ = inputs
    ...
    secondary_burst_array = eos.sar.io.read_window(image_readers[1], secondary_tiff_roi)
    ...
    assert (
        np.abs(np.abs(resampled_secondary_array) - resampled_secondary_amplitude).mean()
        < 10
    )


def test_debursting(dem, inputs):
    image_readers, primary_bursts_meta, secondary_bursts_meta, ref_metas = inputs
    ...
    assert (
        np.isnan(zoomed_with_fourier_separate).sum() / zoomed_with_fourier_separate.size
        < 0.05
    )
