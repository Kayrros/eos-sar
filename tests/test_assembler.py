...
def test_grd_assembler(cdse_s3_session):
    pol = "vv"
    ...
    product1 = CDSEUnzippedSafeSentinel1GRDProductInfo.from_product_id(
        catalog_backend, cdse_s3_session, product_id1
    )
    product2 = CDSEUnzippedSafeSentinel1GRDProductInfo.from_product_id(
        catalog_backend, cdse_s3_session, product_id2
    )
    products = [product1, product2]
    asm = sentinel1.assembler.Sentinel1GRDAssembler.from_products(
        products, pol, statevectors=None
    )
...
def test_projection_and_localization(cdse_s3_session):
    ...
    cols_grid_model, rows_grid_model = np.meshgrid(
        np.linspace(cols[0], cols[1], 100),
        np.linspace(rows_model[0], rows_model[1], 100),
    )

    cols_grid_model2, rows_grid_model2 = np.meshgrid(
        np.linspace(cols[0], cols[1], 100),
    ...
    lon_model2, lat_model2, alt_model2 = proj_model2.localization(
        r_model2, c_model2, alts
    )

    # project the localized points in both proj_model frame to assess the error: Earth --> sat
    rows_pred_model, cols_pred_model, _ = proj_model.projection(
        lon_model, lat_model, alt_model
    )
    rows_pred_model2, cols_pred_model2, _ = proj_model.projection(
        lon_model2, lat_model2, alt_model2
    )
...
def test_grd_assembler_start_of_datatake(cdse_s3_session):
    pol = "vv"
    ...
    asm1 = sentinel1.assembler.Sentinel1GRDAssembler.from_products(
        products, pol, statevectors=None
    )
    asm2 = sentinel1.assembler.Sentinel1GRDAssembler.from_products(
        products, pol, statevectors=None, startend_datatake_cut=False
    )
...
def test_grd_assemble_metadata(cdse_s3_session):
    ...
    combined = sentinel1.metadata.assemble_multiple_grd_products_into_meta(
        (meta1, meta2)
    )
