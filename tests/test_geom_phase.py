...
def test_geom_phase_prediction():
    ...
    heights = eos.sar.dem_to_radar.dem_radarcoding(dem, primary_swath_model,
    ...
    sparse_flat = topo.sparse_flat_earth(rows + row_orig, cols + col_orig,
                                         wrapped=True)
    assert not np.any(sparse_flat[0] - flat_earth[0][rows, cols])

    # test sparse topo prediction
    sparse_topo = topo.sparse_topo_phase(heights[rows, cols], rows + row_orig,
                                         cols + col_orig, wrapped=False)
    assert not np.any(sparse_topo[0] - topo_phase[0][rows, cols])

    # test on multiple secondary imgs
    list_topo = eos.sar.geom_phase.TopoCorrection(primary_swath_model,
                                                  [secondary_swath_model,
                                                   secondary_swath_model],
                                                  grid_size=50, degree=7,
                                                  )
    ...
    check_list_func(list_topo.flat_earth_image, flat_earth[0],
                    "Error list flat earth img",
                    primary_swath_roi, wrapped=True)

    check_list_func(list_topo.topo_phase_image, topo_phase[0],
                    "Error list topo phase img",
                    heights, primary_swath_roi, wrapped=False)

    check_list_func(list_topo.sparse_flat_earth, sparse_flat[0],
                    "Error list sparse flat earth",
                    rows + row_orig, cols + col_orig, wrapped=True)

    check_list_func(list_topo.sparse_topo_phase, sparse_topo[0],
                    "Error list sparse topo phase",
                    heights[rows, cols], rows + row_orig, cols + col_orig,
                    wrapped=False)
