...
class CapellaMetadata:
    def parse_metadata(self, path_to_image, product_type):
        ...
        with open(path_to_image + "_extended.json") as json_file:
            ...
            self.start_timestamp = pd.to_datetime(
                data["collect"]["start_timestamp"], format="%Y-%m-%dT%H:%M:%S.%fZ"
            )
            self.stop_timestamp = pd.to_datetime(
            ...
            self.incidence_angle = float(
                data["collect"]["image"]["center_pixel"]["incidence_angle"]
            )
            self.look_angle = float(
                data["collect"]["image"]["center_pixel"]["look_angle"]
            )
            self.squint_angle = float(
                data["collect"]["image"]["center_pixel"]["squint_angle"]
            )
            self.center_pixel_target_position = data["collect"]["image"][
                "center_pixel"
            ]["target_position"]

            if product_type == "gec":
                self.alt_inflated_wgs84 = float(
                    data["collect"]["image"]["terrain_models"]["reprojection"][
                        "name"
                    ].split("[")[1][:-1]
                )

            if product_type == "slc":
                self.starting_range = float(
                    data["collect"]["image"]["image_geometry"]["range_to_first_sample"]
                )
                self.range_pixel_size = float(
                    data["collect"]["image"]["image_geometry"]["delta_range_sample"]
                )
            self.pixel_spacing_column = float(
                data["collect"]["image"]["pixel_spacing_column"]
            )
            self.range_resolution = float(data["collect"]["image"]["range_resolution"])
            self.ground_range_resolution = float(
                data["collect"]["image"]["ground_range_resolution"]
            )
            self.range_looks = float(data["collect"]["image"]["range_looks"])

            self.azimuth_pixel_size = float(
                data["collect"]["image"]["pixel_spacing_row"]
            )
            self.azimuth_resolution = float(
                data["collect"]["image"]["azimuth_resolution"]
            )
            ...
            self.range_sampling_frequency = float(
                data["collect"]["radar"]["sampling_frequency"]
            )
            self.center_frequency = float(data["collect"]["radar"]["center_frequency"])
            self.wavelength = C / self.center_frequency

            self.transmit_polarization = data["collect"]["radar"][
            ...
            if product_type == "slc":
                ...
                self.last_line_time = self.first_line_time + datetime.timedelta(
                    seconds=self.file_length * self.delta_line_utc
                )
                self.last_line_utc = UTC_time_since_midnight(self.last_line_time)

            self.center_pixel_time = pd.to_datetime(
            ...
            for p in data["collect"]["radar"]["time_varying_parameters"]:
                for mytime in p["start_timestamps"]:
                    radar_info_tim.append(
                        UTC_time_since_midnight(
                            pd.to_datetime(mytime, format="%Y-%m-%dT%H:%M:%S.%fZ")
                        )
                    )
