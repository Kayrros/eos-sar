...
def get_neighbors(ps_col: NDArray[int], ps_row: NDArray[int],
                  distance_threshold: float,
                  resolution_x: float, resolution_y: float) -> NDArray[bool]:
    num_PS = len(ps_col)
    # find neighbors
    distance_x = np.abs(ps_col.reshape([1, -1]) - ps_col.reshape([-1, 1]))
    distance_y = np.abs(ps_row.reshape([1, -1]) - ps_row.reshape([-1, 1]))

    distance_in_meters = np.sqrt((resolution_x * distance_x)**2 +
                                 (resolution_y * distance_y)**2
                                 )
...
def phi_ps_neighbors(phi_sparse, is_at_ok_distance):
    ...
    for k in range(phi_sparse.shape[1]):
        neighbors = is_at_ok_distance.indices[is_at_ok_distance.indptr[k]
            :is_at_ok_distance.indptr[k+1]]
...
def compute_phi_neighbors(ps_col, ps_row, phi_sparse_ts,
                          resolution_x, resolution_y, distance_threshold=300):
...
def compute_ps_vs_neighbors(ps_col, ps_row, phi_sparse_ts,
                            resolution_x, resolution_y, distance_threshold=300):
    phi_sparse_ts = np.array(phi_sparse_ts)
    phi_neighbors = compute_phi_neighbors(ps_col, ps_row, phi_sparse_ts,
                             resolution_x, resolution_y, distance_threshold=distance_threshold)
