...
class periodogram_cl:
    def __init__(self, ctx=None, queue=None,
        ...
        compute_periodograms.set_scalar_arg_dtypes([None, None, None, None, np.int32, np.int32, np.int32])
    ...
    def find_maximum_on_grid(self, constants, variables, weights):
        ...
        assert(constants.shape[2] == self.num_constants_per_sum_term and len(constants.shape) == 3)
        assert(variables.shape[1] == self.num_variables_per_sum_term and len(variables.shape) == 2)
        ...
        constants_cl = cl.Buffer(self.ctx, mf.READ_ONLY, num_ps * sum_size * self.num_constants_per_sum_term * self.dtype_size)
        variables_cl = cl.Buffer(self.ctx, mf.READ_ONLY, num_values_to_test * self.num_variables_per_sum_term * self.dtype_size)
        ...
        # Upload the inputs
        profile(self.events, cl.enqueue_copy(self.queue, constants_cl, constants.flatten(), device_offset=0), "enqueue_copy", False)
        profile(self.events, cl.enqueue_copy(self.queue, variables_cl, variables.flatten(), device_offset=0), "enqueue_copy", False)
        profile(self.events, cl.enqueue_copy(self.queue, weights_cl, weights.flatten(), device_offset=0), "enqueue_copy", False)
        ...
        profile(self.events, self.compute_periodograms(self.queue,
                      global_size,
                      local_size,
                      results_cl,
                      constants_cl,
                      ...
                      num_values_to_test,
                      sum_size),
               "compute_periodograms", True)

        # Wait the results and download them
        results = np.empty([num_ps * 2], dtype=self.dtype, order='C')
        profile(self.events, cl.enqueue_copy(self.queue, results, results_cl), "enqueue_copy", False)
