...
def get_gcps_localization(proj_model, grid_size=10):
    ...
    Cols, Rows = np.meshgrid(np.linspace(0, w, num=grid_size), np.linspace(0, h, num=grid_size))
    rows = Rows.ravel()
    cols = Cols.ravel()
    lons, lats, alts, _ = proj_model.localize_without_alt(rows, cols)
    gcps = [rasterio.control.GroundControlPoint(row, col, x, y, z) for row, col, x, y, z in zip(rows, cols, lons, lats, alts)]
    return gcps
...
def geometry_to_roi(geom_coords, proj_model, dem: DEM,
...
class GeometryRoiProvider(RoiProvider):
    ...
    def get_roi(self, proj_model: SensorModel, dem_source: DEMSource):
        ...
        # Get the roi
        roi, rows, cols = geometry_to_roi(geo_coords, proj_model, dem=dem,
...
class CentroidRoiProvider(RoiProvider):
    ...
    def get_roi(self, proj_model: SensorModel, dem_source: DEMSource):
        dem_bounds = shapely.Point(*self.point).buffer(self.dem_fetch_buffer_round_point).bounds
        dem = dem_source.fetch_dem(dem_bounds)
...
class RoiCuttingInfo:
    ...
    def to_dict(self) -> dict:
        return dict(all_bsids=list(self.all_bsids),
...
class Registrator:
    ...
    def __init__(self, primary_proj_model, roi_for_dem, bsids, primary_cutter,
                 dem: eos.dem.DEM, dem_sampling_ratio=1,
                 bistatic=True, apd=True, intra_pulse=True, alt_fm_mismatch=True
                 ):
        ...
        x, y, alt, crs = eos.sar.regist.get_registration_dem_pts(
            primary_proj_model, roi=roi_for_dem, margin=0, dem=dem,
    ...
    def estimate_primary_regist(self, primary_correctors_povider):
        ...
        correctors = self._get_correctors(primary_correctors_povider,
                                          self.bsids)
        ...
        for bsid in self.bsids:
            ...
            geo_im_pt = eos.sar.projection_correction.GeoImagePoints(
                self.gx[burst_mask], self.gy[burst_mask], self.gz[burst_mask],
                self.azt_primary_no_correc[bsid], self.rng_primary_no_correc[bsid])
        ...
        burst_resampling_matrices = eos.products.sentinel1.regist.get_burst_resampling_matrices(
            self.primary_cutter, self.primary_cutter,
            self.azt_primary_no_correc, self.rng_primary_no_correc,
            azt_primary_correc, rng_primary_correc, self.bsids)

        return burst_resampling_matrices

    def estimate_secondary_regist(self, secondary_cutter, secondary_bsids,
        ...
        burst_resampling_matrices = eos.products.sentinel1.regist.secondary_registration_estimation(
            secondary_proj_model, secondary_cutter, secondary_corrector_per_bsid,
            self.gx, self.gy, self.gz, self.crs,
            bsids, self.pts_in_burst_mask, self.primary_cutter,
            self.azt_primary_no_correc, self.rng_primary_no_correc)
...
class Deburster:
    ...
    def deburst(self, secondary_cutter, secondary_resampler_provider,
                burst_resampling_matrices, secondary_readers, get_complex=True, reramp=True):
        ...
        _, read_rois_correc, resamplers_on_rois = eos.products.sentinel1.deburst.warp_rois_read_resample_deburst(
            bsids, resamplers, self.roi_cutting_info.within_burst_rois, secondary_cutter,
            secondary_readers, self.roi_cutting_info.write_rois, out_shape, out,
...
class Ifg:
    ...
    def get_flattened(self):
        if self.flattened is None:
            flattening_terms = [self.get_flattening_term(*self.dir_reader.read_flat_phase(self.dates))]
            self.flattened = self.apply_flattening_terms(self.get_init_interf(), flattening_terms)
        return self.flattened

    def get_topo_corrected(self):
        if self.topo_corrected is None:
            flattening_terms = [self.get_flattening_term(*self.dir_reader.read_topo_phase(self.dates))]
            self.topo_corrected = self.apply_flattening_terms(self.get_flattened(), flattening_terms)
        return self.topo_corrected

    def multilook(self, interf, filter_size=(2, 8), compute_coherence=False, undersample=True):
        return multilook(interf, filter_size,
                         self.amp1, self.amp2,
                         compute_coherence, undersample)


def filt_interf(interf, filter_size=(5, 5), fft_size=32, window_size=5, alpha=0.5, nworkers=1):
    mlooked, _ = multilook(interf, filter_size)
    filt = goldstein_filter.apply(
        mlooked, fft_size, window_size, alpha, nworkers)
    return filt

def estimate_corrections(primary_proj_model, roi, secondary_proj_model, heights):

    topo = eos.sar.geom_phase.TopoCorrection(primary_proj_model,
                                             [secondary_proj_model],
...
def multilook(interf, filter_size=(1, 4), primary_amp=None, secondary_amp=None,
              compute_coherence=False, undersample=False):
    ...
    if compute_coherence:
        ...
        coherence = np.abs(mlooked) / (np.sqrt(compute_filtered_magnitude(primary_amp, filter_size)\
...
class OvlIfg(Ifg):
    ...
    def get_flattened(self):
        if self.flattened is None:
            flattening_terms = [self.get_flattening_term(*self.dir_reader.read_flat_phase(self.osid.bsint, self.dates))]
            self.flattened = self.apply_flattening_terms(self.get_init_interf(), flattening_terms)
        return self.flattened

    def get_topo_corrected(self):
        if self.topo_corrected is None:
            flattening_terms = [self.get_flattening_term(*self.dir_reader.read_topo_phase(self.osid.bsint, self.dates))]
            self.topo_corrected = self.apply_flattening_terms(self.get_flattened(), flattening_terms)
        return self.topo_corrected
