...
class PrimaryPipeline(Pipeline):
    ...
    def roi_info(self, roi_provider: utils.ProjectionRoiProvider):
        ...
        inout.imcoords_to_svg(zip(cols - c_orig, rows - r_orig), self.dir_builder.get_svg_path())

        print("ROI: ", self.roi)

        print("Roi cutter instanciation")
        self.primary_cutter = self.asm.get_primary_cutter()
        self.roi_cutting_info = utils.RoiCuttingInfo.from_cutter_roi(self.primary_cutter, self.roi)
    ...
    def register(self, dem_sampling_ratio, bistatic, apd, intra_pulse, alt_fm_mismatch):
        ...
        self.registrator = utils.Registrator(
            self.proj_model, self.roi_cutting_info.roi,
            self.roi_cutting_info.all_bsids, self.primary_cutter,
            dem=self.dem, dem_sampling_ratio=dem_sampling_ratio, bistatic=bistatic, apd=apd,
    ...
    def deburst(self, polarization, calibrate, get_complex):
        ...
        debursted_crop, read_rois_src, resamplers_on_roi = self.deburster.deburst(
            self.primary_cutter, self.asm.get_burst_resampler,
            self.burst_resampling_matrices, readers,
            get_complex, reramp=True)
    ...
    def radarcode_dem(self):
        ...
        self.heights = eos.sar.dem_to_radar.dem_radarcoding(self.dem,
                                                       self.proj_model,
                                                       roi=self.roi)
...
class SecondaryPipeline(Pipeline):
    ...
    def register(self, registrator):
        ...
        self.burst_resampling_matrices = registrator.estimate_secondary_regist(
            self.cutter, bsids, self.proj_model,
            correctors_provider)
    ...
    def deburst(self, deburster, polarization, calibrate, get_complex):
        ...
        readers = self.asm.get_image_readers(
           self.products, self.burst_resampling_matrices.keys(),
           polarization, calibrate)

        debursted_crop, read_rois_src, resamplers_on_roi = deburster.deburst(
            self.cutter,  self.asm.get_burst_resampler,
            self.burst_resampling_matrices, readers, get_complex, reramp=True)
    ...
    def execute(self, product_provider, orbit_provider, polarization,
                registrator, deburster, calibrate,
                get_complex, primary_proj_model, roi, heights):
        self.get_inputs(product_provider, orbit_provider, polarization)
...
class OvlPrimaryPipeline(Pipeline):
    ...
    def get_ovl_roi_in_swath(self, bsint):
        ...
        swath = bsint.bsids()[0].split("_")[1].lower()
        ovl_roi_in_swath = self.ovl_roi_info_per_swath[swath].get_swath_rois_per_bsint()[bsint]
    ...
    def get_bsint_coarse_geom(self, bsint, margin=1000, alt_min=-1000, alt_max=9000):
        ...
        return swath_model.get_coarse_approx_geom(ovl_roi_in_swath,
                                                  margin=margin,
                                                  alt_min=alt_min, alt_max=alt_max)

    def get_bsints_of_interest_coarse_bounds(self, margin=1000, alt_min=-1000, alt_max=9000):
        lons = []
        lats = []
        for bsint in self.bsint_of_interest:
            geom = self.get_bsint_coarse_geom(bsint, margin=margin,
                                              alt_min=alt_min, alt_max=alt_max)
    ...
    def download_dem(self):
        self.dem = self.dem_source.fetch_dem(self.get_bsints_of_interest_coarse_bounds())
    ...
    def register(self, dem_sampling_ratio, bistatic, apd, intra_pulse, alt_fm_mismatch):
        ...
        for swath in self.swaths_of_interest:
            ...
            bounds = tuple(np.amin(coords[:, 0:2], axis=0)) + tuple(np.amax(coords[:,2:4], axis=0))
            roi_for_dem = eos.sar.roi.Roi.from_bounds_tuple(bounds)
            self.registrator_per_swath[swath] = utils.Registrator(
                self.swath_models_per_swath[swath], roi_for_dem, self.bsids_of_interest_per_swath[swath],
                self.primary_cutter, dem=self.dem, dem_sampling_ratio=dem_sampling_ratio,
                bistatic=bistatic, apd=apd, intra_pulse=intra_pulse, alt_fm_mismatch=alt_fm_mismatch)

            print("Primary registration estimation")
            self.burst_resampling_matrices.update(self.registrator_per_swath[swath].estimate_primary_regist(
                self.asm.get_corrector_per_bsid))

    def resample_swath_ovls(self, swath, polarization, calibrate, get_complex, reramp):
        swath_model = self.swath_models_per_swath[swath]
        # this will work on all overlaps in a swath
        overlap_resampler = OverlapResampler(self.ovl_roi_info_per_swath[swath], self.primary_cutter)
        ...
        all_resampled_ovls, _, all_resamplers = overlap_resampler.resample(
            self.osids_of_interest_per_swath[swath], self.burst_resampling_matrices,
            self.asm.get_burst_resampler, self.primary_cutter, readers,
    ...
    def execute(self, product_provider, orbit_provider, dem_sampling_ratio, bistatic, apd, intra_pulse, alt_fm_mismatch,
                polarization, calibrate, get_complex, reramp, swaths=("iw1", "iw2","iw3"), osids_of_interest=None):
        self.get_inputs(product_provider, orbit_provider, polarization)
        ...
        # loop on the swaths
        for swath in self.swaths_of_interest:
            self.resample_swath_ovls(swath, polarization, calibrate, get_complex, reramp)
        ...
        self.log["overlap_roi_info"] = {k: v.to_dict() for k, v in self.ovl_roi_info_per_swath.items()}
        self.log["osids_of_interest_per_swath"] = {k: [str(_v)  for _v in v] for k, v in self.osids_of_interest_per_swath.items()}
        self.log["bsint_of_interest_per_swath"] = {k: [str(_v)  for _v in v] for k, v in self.bsint_of_interest_per_swath.items()}
        self.log["resampler_per_osid"] = {str(k): v.to_dict() for k, v in self.resampler_per_osid.items()}
...
class OvlSecondaryPipeline(Pipeline):
    ...
    def register(self, registrator_per_swath):
        ...
        for swath, registrator in registrator_per_swath.items():
            ...
            self.secondary_burst_resampling_matrices.update( registrator.estimate_secondary_regist(
                self.secondary_cutter, self.asm.bsids, self.swath_models_per_swath[swath],
                secondary_correctors_provider) )

    def resample_swath_ovls(self, overlap_resampler, swath, polarization,
                            calibrate, get_complex, reramp, primary_osids_of_interest):
        print("Resampling on secondary for swath ", swath)
        secondary_swath_model = self.swath_models_per_swath[swath]
        swath_osid_set = set(primary_osids_of_interest).intersection(secondary_swath_model.osids)

        osids_of_interest = sorted(list(swath_osid_set), key=lambda x: x.bsid())

        self.osids_of_interest_per_swath[swath] = osids_of_interest

        self.bsint_of_interest_per_swath[swath] = set([o.bsint for o in osids_of_interest])
        ...
        all_resampled_ovls_sec, _, all_resamplers_sec = overlap_resampler.resample(
            osids_of_interest, self.secondary_burst_resampling_matrices, self.asm.get_burst_resampler,
            self.secondary_cutter, secondary_readers, get_complex=get_complex, reramp=reramp)

        self.resampler_per_osid.update(all_resamplers_sec)

        assert set(all_resampled_ovls_sec.keys()) == set(osids_of_interest), "osids should not change after resampling"
    ...
    def simulate_phase_per_swath(self, primary_swath_model, height_provider,
                                 bsint_of_interest, ovl_roi_in_swath_per_bsint):
        ...
        for bsint in bsint_of_interest:
            ...
            topo_phase = topo.topo_phase_image(height_provider(bsint),
                                               primary_roi=ovl_roi_in_swath)[0]

            inout.save_img(self.dir_builder.get_flat_path(bsint, self.date), flat_earth_phase)
            inout.save_img(self.dir_builder.get_topo_path(bsint, self.date), topo_phase)

    def execute(self, product_provider, orbit_provider, registrator_per_swath,
                overlap_resamplers_per_swath, polarization,
                calibrate, get_complex, reramp, primary_osids_of_interest_per_swath,
                primary_swath_model_per_swath, height_provider, ovl_roi_in_swath_per_bsint,
                ):
        ...
        for swath in primary_osids_of_interest_per_swath.keys():
            self.resample_swath_ovls(overlap_resamplers_per_swath[swath], swath, polarization,
                                    calibrate, get_complex, reramp, primary_osids_of_interest_per_swath[swath])

            print("Simulating phase for swath ", swath)
            self.simulate_phase_per_swath(primary_swath_model_per_swath[swath], height_provider,
                                          self.bsint_of_interest_per_swath[swath], ovl_roi_in_swath_per_bsint)
