...
"""
def save_debug_image(path, PS_X_coordinates, PS_Y_coordinates, parent_shape,
                     sparse_data, as_complex=True):
    data_full = psutils.sparse_data_to_raster(sparse_data, PS_Y_coordinates, PS_X_coordinates,
                                parent_shape)
    if as_complex:
...
def iterative_alternate_periodogram(PS_X_coordinates, PS_Y_coordinates,
                                   Delta_phi_against_ref, bperp, inc, rng, dates, max_iterations = 10,
                                   threshold_q=0.7, threshold_v=0.1, wavelength=5.5465763*1e-2,
                                   debug_path=None, *, use_tensorflow=True):
    ...
    times_differences_against_ref = [(d-dates[0]).days / 365.25 for d in dates[1:]] # total_seconds() to get seconds
    ...
    PS_Delta_phi_against_ref = Delta_phi_against_ref[:, PS_Y_coordinates, PS_X_coordinates]
    ...
    APS_dzeta_model = periodogram.LinearTermModel(1., PS_Y_coordinates, np.linspace(-0.1, 0.1, 11)) # odd boundaries to have 0. tested
    APS_eta_model = periodogram.LinearTermModel(1., PS_X_coordinates, np.linspace(-0.1, 0.1, 11))
    ...
    for iteration in range(max_iterations):
        ...
        if iteration > 1 and max(abs(delta_q)) < threshold_q and max(abs(delta_v)) < threshold_v:
            break
        ...
        Delta_phi_no_q_v_estimation = get_phi_no_q_v_estimation(PS_Delta_phi_against_ref, Cq, date_normal_baseline, q_estimation, Cv, times_differences_against_ref, v_estimation)
        ...
        periodogram_for_each_date = np.exp(1j * (Delta_phi_no_q_v_estimation[:, :] - p_dzeta[:, np.newaxis] * PS_Y_coordinates[np.newaxis,:] - p_eta[:, np.newaxis] * PS_X_coordinates[np.newaxis,:]))
        ...
        APS_estimated = ak[:, np.newaxis] + p_dzeta[:, np.newaxis] * PS_Y_coordinates[np.newaxis,:] + p_eta[:, np.newaxis] * PS_X_coordinates[np.newaxis,:]
        ...
        if debug_path is not None:
            ...
            save_debug_image(os.path.join(debug_path, "APS_%d.tiff" % iteration)
                            , PS_X_coordinates, PS_Y_coordinates, parent_shape, APS_estimated[-1, :])
            save_debug_image(os.path.join(debug_path, "DPHI_NOMVT_NOTOPO_%d.tiff" % iteration),
                             PS_X_coordinates, PS_Y_coordinates, parent_shape, Delta_phi_no_q_v_estimation[-1, :])
            save_debug_image(os.path.join(debug_path, "DPHI_TOPO_%d.tiff" % iteration),
                             PS_X_coordinates, PS_Y_coordinates, parent_shape,(-Cq * date_normal_baseline * q_estimation[np.newaxis, :])[-1, :])
            save_debug_image(os.path.join(debug_path, "DPHI_MVT_%d.tiff" % iteration),
                             PS_X_coordinates, PS_Y_coordinates, parent_shape,(Cv * times_differences_against_ref[:, np.newaxis] * v_estimation[np.newaxis, :])[-1, :])
            save_debug_image(os.path.join(debug_path, "DPHI_NOAPS_NOMVT_NOTOPO_%d.tiff" % iteration),
                             PS_X_coordinates, PS_Y_coordinates, parent_shape, Delta_phi_estimation_noplane[-1, :])
        ...
        delta_q, delta_v, _ = velo_topo_periodogram(
            Delta_phi_estimation_noplane, Cq, date_normal_baseline, Cv, times_differences_against_ref,
            date_coefs, use_tensorflow=use_tensorflow)
...
def velo_topo_periodogram(phi_ps_mat, Cq, date_normal_baseline, Cv, times_differences_against_ref, weights_per_date=None,
                          *, use_tensorflow=True):
    ...
    if use_tensorflow:
        ...
        def periodogram_to_optimize(x):
            ...
            for k in range(num_dates):
                res += weights_per_date[k] * tf.exp(tf.dtypes.complex(tf.cast(0., tf.float64), (tf_phi_ps_mat[k, :] - tf.cast(Cq * date_normal_baseline[k], tf.float64) * q - tf.cast(Cv * times_differences_against_ref[k], tf.float64) * v)))
            return res
        ...
        res = tfp.optimizer.lbfgs_minimize(
            ...
            tolerance=1e-15, max_iterations=10)
        x = res.position
        q = x[0:num_PS].numpy()
        v = x[num_PS:].numpy()
        ...
        lin_defo_model = periodogram.LinearTermModel(Cv, times_differences_against_ref, v_test)
        for h in range(num_PS):
            topo_model = periodogram.LinearTermModel(Cq[0, h], date_normal_baseline[:, h], q_test)
            defo_topo_model = periodogram.CompoundModel([lin_defo_model,
                                                         topo_model])
...
def spatial_low_pass_interpolate_atmo(residual, PS_X_coordinates, PS_Y_coordinates,
                                      resolution_x, resolution_y, parent_shape,
                                      distance_threshold=100, interpolation_method="cubic"):
    # low pass spatially
    phi_low_pass = neighbors.compute_phi_neighbors(PS_X_coordinates, PS_Y_coordinates,
                                                        residual, resolution_x, resolution_y,
    ...
    for i, phi_nei in enumerate(phi_low_pass):
        # interpolate
        interp = scipy.interpolate.griddata((PS_X_coordinates, PS_Y_coordinates),
                                            phi_nei,
                                            (np.arange(w)[None, :], np.arange(h)[:, None]),
                                            method=interpolation_method, fill_value=np.nan, rescale=False)
...
def get_atmo_full(interpolated, ak, pdzeta, peta, parent_shape):
    ...
    atmos = ak[:, None, None] + pdzeta[:, None, None] * yy + peta[:, None, None] * xx + interpolated
    return atmos

def final_periodogram(phi_ts_raster, atmos, dates, rng, inc, bperp,
                      wavelength=5.5465763*1e-2, *, use_tensorflow=True):
    ...
    q, v, gammas = velo_topo_periodogram(
        phi_no_atmo_sparse, Cq, date_normal_baseline, Cv, times_differences_against_ref,
        use_tensorflow=use_tensorflow)
    phi_residual = get_phi_no_q_v_estimation(phi_no_atmo_sparse, Cq, date_normal_baseline, q, Cv, times_differences_against_ref, v)
    ...
    defo_linear = times_differences_against_ref[:,np.newaxis] * v[np.newaxis,:] # (mm)
    return q, v, gammas, col, row, defo_nonlinear, defo_linear

def get_phi_no_q_v_estimation(phi_ps_mat, Cq, date_normal_baseline, q, Cv, times_differences_against_ref, v):
    phi_no_q_v_estimation = phi_ps_mat - Cq * date_normal_baseline * q[np.newaxis,:] - Cv * times_differences_against_ref[:,np.newaxis] * v[np.newaxis,:]
    return phi_no_q_v_estimation

def full_pipeline(amps, phi_ts, bperp, inc, rng, dates, resolution_x, resolution_y, da_threshold=0.25,
                  max_iterations=10, threshold_q=0.7, threshold_v=0.1, first_gamma_threshold=0.8, second_gamma_threshold=0.9,
                  distance_threshold=300, interpolation_method="cubic", wavelength=5.5465763*1e-2,
                  *, use_tensorflow=True): # no clean, some things hardcoded
    print("ps candidates selection")
    # ps candidates
    _, PS_candidates_basic, PS_candidates_mask = psc.get_PS_candidates_DA(amps, da_threshold)
    ...
    q_estimation, v_estimation, APS_estimated, ak, pdzeta, peta, residual = iterative_alternate_periodogram(
            PS_X_coordinates, PS_Y_coordinates, Delta_phi_against_ref, bperp,
            inc, rng, dates, max_iterations=max_iterations, threshold_q=threshold_q,
            threshold_v=threshold_v, wavelength=wavelength,
            use_tensorflow=use_tensorflow)
    ...
    interpolated = spatial_low_pass_interpolate_atmo(
         residual[:, good_ps], PS_X_coordinates[good_ps], PS_Y_coordinates[good_ps],
         resolution_x, resolution_y, parent_shape=Delta_phi_against_ref[0].shape,
         distance_threshold=distance_threshold, interpolation_method=interpolation_method)

    # add affine planes to interpolated to get the full atmo
    atmos = get_atmo_full(interpolated, ak, pdzeta, peta,
                          parent_shape=Delta_phi_against_ref[0].shape)

    print("final periodogram")
    # do final periodogram
    q, v, gammas, col, row, defo_nonlinear, defo_linear = final_periodogram(
        Delta_phi_against_ref, atmos, dates, rng, inc, bperp, wavelength=wavelength,
        use_tensorflow=use_tensorflow)

    # keep only good ps
    final_ps_mask = gammas > second_gamma_threshold

    return q[final_ps_mask], v[final_ps_mask], gammas[final_ps_mask], col[final_ps_mask], row[final_ps_mask], \
        defo_nonlinear[:, final_ps_mask], defo_linear[:, final_ps_mask]
