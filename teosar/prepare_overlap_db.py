...
def to_ovl_arr(array, overlap_roi_info, osid):
    return write_array(array, overlap_roi_info.all_write_rois[osid], overlap_roi_info.all_out_shapes[osid])
...
def get_f_dop(resampler_per_osid, overlap_roi_info_per_swath, osid):
    ...
    eta, ref_time, dop_centroid, dop_rate = resampler.get_doppler_params_gridded(
        np.arange(resampler.dst_shape[0]), np.arange(resampler.dst_shape[1]),
...
def normalize_cmpx_values(cmpx_values):
    ...
    normalized[non_zero_mask] = normalized[non_zero_mask] / \
        (amp[non_zero_mask] + 1e-12)
    return normalized


def main(dstdir, product_ids_per_date, orbit_type,  polarization,
         calibrate, get_complex, bistatic, apd,
         intra_pulse, alt_fm_mismatch, dem_sampling_ratio,
         primary_id, osids_of_interest=None):

    pipelines = tsinsar.main_ovl(dstdir, product_ids_per_date,
                                 orbit_type,
                                 polarization,
                                 calibrate,
                                 get_complex,
    ...
    az_frequency = primary_pipeline.swath_models_per_swath["iw1"].coordinate.azimuth_frequency

    px_shift_per_bsint = {}

    for swath in swaths:
        bsint_of_interest_in_swath = secondary_pipeline.bsint_of_interest_per_swath[swath]

        for bsint in bsint_of_interest_in_swath:

            # Do the forward and backward interferograms
            osids = bsint.osids()
            ifg_per_osid = {osid: OvlIfg(
                dir_reader, *dates, osid) for osid in osids}

            # here we should have only two osids per bsint
            init_ifg = {
                osid: ifg_per_osid[osid].get_init_interf() for osid in osids}
            ovl_interf = init_ifg[osids[0]] * np.conj(init_ifg[osids[1]])

            f_dop = {osid: get_f_dop(
                resampler_per_osid, overlap_roi_info_per_swath, osid) for osid in osids}
            ...
            px_shift = np.nanmean(
                phi_agg / (K_shift_to_phase[valid_mask] + 1e-12))

            px_shift_per_bsint[str(bsint)] = px_shift

            for osid in osids:

                esd_correction = np.exp(-1j * 2 * np.pi *
                                        f_dop[osid] / az_frequency * px_shift, dtype=np.complex64)

                before_esd = ifg_per_osid[osid].get_topo_corrected()
                inout.save_img(dir_builder.get_ifg_path(
                    osid, *dates), before_esd)

                # px_shift_per_bsint[bsint] = px_shift
                esd_corrected = before_esd * esd_correction

                inout.save_img(dir_builder.get_ifg_path(
                    osid, *dates, esd=True), esd_corrected)

    inout.dict_to_json(px_shift_per_bsint,
                       dir_builder.get_ifg_meta_path(*dates))
