...
def get_ps_masks_on_win(search_window, guard_window,
    ...
    ps_bw = np.logical_and(ps_candidates_full_array,
                                             mask_search_outside_guard)

    ps_in = np.logical_and(ps_candidates_full_array,
                                     mask_guard)


    ps_out = np.logical_and(ps_candidates_full_array,
                                       ~mask_search)

    return MaskedPS(ps_in, ps_bw, ps_out)

def compensate_plane_from_surround(search_window, guard_window, ps_candidates_full_array,
                    phi_ts, max_slopes=(5e-2, 5e-2), min_half_samples=10, *, no_failure=False, result_only_inside=False):
    # masking in windows
    print("Masking PS")
    ...
    model = periodogram.get_planar_model(xx, yy, max_slopes=max_slopes, min_half_samples=min_half_samples)
    # solving
    ...
    phi_img_compensated_ts = np.array([psutils.sparse_data_to_raster(phi_ps, row, col, parent_shape)
